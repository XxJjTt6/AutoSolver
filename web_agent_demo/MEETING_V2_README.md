...
| **第3幕 自主学习** | 进化的是"对状况的识别"，不是求解器 | 因果条①看场景→②挑打法→③结果更好；六阶段 Stage Rail；28 条真实事件逐条回放；「学到了什么」(best-so-far 真阶梯 2097→683→657，默认展开) + 「🧠 三层记忆 L0/L1/L2」+ 「🛡 成绩单」(5 策略全被拒=质量门有判别力) + 「评委可能会问」Q&A |

导航：顶部三幕按钮，或「下一步 ▶」依次推进。

**键盘快捷键**：`1/2/3` 跳到对应幕；`←/→` 在第3幕翻事件、其余幕翻幕；`Esc` 关引导。
