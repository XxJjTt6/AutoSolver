"""v9 启动器：基于 v8，决策页只展示当前时刻已经实际发生的轮次。
复用原 server.py 的全部后端逻辑，仅把首页渲染替换为 day_replay_frontend_v9.render_day_replay_index。
...
# 允许以脚本方式直接运行（python3 web_agent_demo/server_v9.py）。
...
# 必须在首次渲染/首次跑仿真之前 apply（day_replay_frontend_v9 的 _bootstrap_payload 惰性缓存首个请求时才跑仿真）。
...
from web_agent_demo.day_replay_frontend_v9 import render_day_replay_index  # noqa: E402
...
def _start_roster_recalc() -> None:
    def _worker(gen: int) -> None:
        ...
        from web_agent_demo import day_replay_frontend_v9 as fe
        try:
