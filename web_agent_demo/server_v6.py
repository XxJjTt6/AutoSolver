"""v6 启动器：合并版 —— v5 的「慢单率」指标 + v3 后续的全站修复（编号统一、因果口径、订单池/决策页防泄漏、双屏累计收益卡）。
复用原 server.py 的全部后端逻辑，仅把首页渲染替换为 day_replay_frontend_v6.render_day_replay_index。
不改动 server.py / server_v5.py / day_replay_frontend_v5.py 任何原文件。

用法与原 server 一致：
    python3 web_agent_demo/server_v6.py --host 127.0.0.1 --port 8799
"""
...
# 允许以脚本方式直接运行（python3 web_agent_demo/server_v6.py）。
...
# 必须在首次渲染/首次跑仿真之前 apply（day_replay_frontend_v6 的 _bootstrap_payload 惰性缓存首个请求时才跑仿真）。
...
from web_agent_demo.day_replay_frontend_v6 import render_day_replay_index  # noqa: E402
...
def _start_roster_recalc() -> None:
    def _worker(gen: int) -> None:
        ...
        from web_agent_demo import day_replay_frontend_v6 as fe
        try:
