        ...
        try:
            ...
            if parsed.path == "/api/roster-add":
                if payload.get("type") == "order":
                    added = runtime_roster.add_order(str(payload.get("merchant_id", "")), int(payload.get("created_at_s", 12 * 3600)), str(payload.get("note", "")))
                elif payload.get("type") == "rider":
                    added = runtime_roster.add_rider(str(payload.get("zone_id", "office_core")), int(payload.get("shift_start_s", 7 * 3600)), int(payload.get("shift_end_s", 23 * 3600)), int(payload.get("capacity", 3)))
                else:
