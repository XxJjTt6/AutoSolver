"""v7 启动器：基于 v6，订单池整页因果化（未来订单在下单时刻前完全不可见）+ 顶栏订单数随推演时钟增长。
复用原 server.py 的全部后端逻辑，仅把首页渲染替换为 day_replay_frontend_v7.render_day_replay_index。
...
# 允许以脚本方式直接运行（python3 web_agent_demo/server_v7.py）。
...
# 必须在首次渲染/首次跑仿真之前 apply（day_replay_frontend_v7 的 _bootstrap_payload 惰性缓存首个请求时才跑仿真）。
...
from web_agent_demo.day_replay_frontend_v7 import render_day_replay_index  # noqa: E402
...
def _start_roster_recalc() -> None:
    def _worker(gen: int) -> None:
        ...
        from web_agent_demo import day_replay_frontend_v7 as fe
        try:
